...
function indicatorMethodText(group, name) {
  ...
  if (block?.key === "green") return "Показывает, насколько близко к жителям находятся парки, скверы и другие зелёные зоны.";
  if (block?.key === "commerce") {
...
function indicatorMethodNotes(group, name) {
  ...
  if (block?.key === "green") return ["Показывает не просто наличие зелени на карте, а удобство доступа к ней для жителей.", "Высокое значение означает, что парк или сквер находится рядом, а не только в другой части города."];
  if (block?.key === "commerce") {
